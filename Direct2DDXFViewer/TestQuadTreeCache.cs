...
{
    public class TestQuadTreeCache : IDisposable
    {
        #region Fields
        ...
        #region Properties 
        public ConcurrentDictionary<double, List<TestQuadTree>> QuadTrees { get; private set; }
        public List<TestQuadTree> CurrentQuadTrees { get; private set; }
        public List<TestQuadTree> InitialQuadTrees { get; private set; }
        #endregion
        ...
        public TestQuadTreeCache(RenderTarget renderTarget, Factory1 factory, ObjectLayerManager layerManager, Rect extents, Matrix extentsMatrix, ResourceCache resCache, float zoomFactor)
        {
        ...
        public (List<TestQuadTree> quadTrees, QuadTreeGetterType info) GetQuadTree(float zoom)
        {
            // If the zoom is already in the dictionary, return the quad tree
            if (QuadTrees.TryGetValue(Math.Round(zoom, 3), out List<TestQuadTree> quadTrees))
            {
            ...
            quadTrees = new List<TestQuadTree>() { CreateQuadTree(_renderTarget, zoom, size, _extentsMatrix) };
            QuadTrees.TryAdd(Math.Round((double)zoom, 3), quadTrees);
            return (quadTrees, QuadTreeGetterType.New);
        }

        private TestQuadTree CreateQuadTree(RenderTarget renderTarget, float zoom, Size2F size, Matrix extentsMatrix)
        {
            ...
            _layerManager.DrawVisibleObjectsToRenderTarget(bitmapRenderTarget, thickness);
            bitmapRenderTarget.EndDraw();

            TestQuadTree quadTree = new(renderTarget, bitmapRenderTarget.Bitmap, zoom, _maxBitmapSize, dpi,
        ...
        private List<TestQuadTree> CreateOversizedQuadTrees(RenderTarget renderTarget, float zoom, Size2F fullSize, Matrix extentsMatrix, int maxBitmapRenderTargetSize)
        {
            List<TestQuadTree> quadTrees = new();
            Size2F dpi = new(96.0f * zoom, 96.0f * zoom);
                ...
                for (int i = 0; i < numTilesWidth; i++)
                {
                    for (int j = 0; j < numTilesHeight; j++)
                    {
                        ...
                        try
                        {
                            ...
                            TestQuadTree quadTree = new(_renderTarget, bitmapRenderTarget.Bitmap, zoom, _maxBitmapSize, dpi, treeRect, destRect);
        ...
        /// <returns></returns>
        private async Task GetAdjacentZoomQuadTreesAsync(List<TestQuadTree> currentQuadTrees)
        {
